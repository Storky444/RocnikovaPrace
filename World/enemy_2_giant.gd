...
func start_attack():
	...
	anim.stop()
	anim.play("GiantAnimationAttack")
	play_attack_sound()
...
func update_animation(direction: Vector2):
	if abs(direction.x) > abs(direction.y):
		if direction.x > 0:
			anim.play("GiantAnimationRight")
		else:
			anim.play("GiantAnimationLeft")
	else:
		if direction.y > 0:
			anim.play("GiantAnimationDown")
		else:
			anim.play("GiantAnimationUp")
...
func die() -> void:
	...
	anim.stop()
	anim.play("GiantAnimationDeath")

	await anim.animation_finished
	queue_free()

func _on_health_bar_2_value_changed(value: float) -> void:
	pass
